...
However, inherent challenges greatly limit the performance of the model: **(1)** Tumor appearance difference caused by non-tumor factors. **(2)** Small inter-class difference and large intra-class variations. In this paper, we propose a novel diagnosis framework for renal tumors, Bi-knowledge Contrastive Network (BKC-Net), which has two innovations:
